...
export const getFullSizePfpLink = (small_icon_url: string): string => {
	if (small_icon_url.includes("_normal.")) {
		return small_icon_url.replace("_normal.", "_bigger.");
	} else {
		throw Error("Provided URL did not contain _normal.<ext> image suffix.");
	}
};
 ...
 */
export const highlightTweetTags = (
	tweet_body: string,
	urls?: Array<TweetURL>,
	...
	if (urls) {
		urls.forEach((url) => {
			tweet_body = tweet_body.replace(
				`${url.url}`,
				`<span class="tweet-tag">${url.url}</span>`
			);
		});
	}
	...
	if (hashtags) {
		hashtags.forEach((hashtag) => {
			tweet_body = tweet_body.replace(
				`#${hashtag.tag}`,
				`<span class="tweet-tag">#${hashtag.tag}</span>`
			);
		});
	}
	...
	if (mentions) {
		mentions.forEach((mention) => {
			tweet_body = tweet_body.replace(
				`@${mention.username}`,
				`<span class="tweet-tag">@${mention.username}</span>`
			);
		});
	}
